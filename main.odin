...
main :: proc() {
    ...
    result = vk.EnumerateInstanceExtensionProperties(nil, &extension_count, raw_data(extension_list))
...
init :: proc() -> (app: Hello_Triangle) {
    ...
    for image, i in app.swap_chain_images {
        ...
        if vk.CreateImageView(app.device, &create_info, nil, &app.swap_chain_image_views[i]) != .SUCCESS {
...
create_swap_chain :: proc(app: ^Hello_Triangle) -> vk.SwapchainKHR {
    ...
    vk.GetSwapchainImagesKHR(app.device, swap_chain, &sc_image_count, raw_data(app.swap_chain_images))
...
choose_swap_extent :: proc(app: Hello_Triangle, capabilities: vk.SurfaceCapabilitiesKHR) -> vk.Extent2D {
...
create_logical_device :: proc(app: Hello_Triangle) -> (dev: vk.Device, graphics, present: vk.Queue) {
    indices := find_queue_families(app, app.physical_device)
...
is_device_suitable :: proc(app: Hello_Triangle, device: vk.PhysicalDevice) -> bool {
    ...
    return  bool(device_features.geometryShader) && is_complete(indices) && extensions_supported && swapchain_adequate
}
...
find_queue_families :: proc(app: Hello_Triangle, device: vk.PhysicalDevice) -> (indices: Queue_Family_Indices) {
...
query_swap_chain_support :: proc(app: Hello_Triangle, device: vk.PhysicalDevice) -> (result: Swap_Chain_Support_Details) {
    ...
    if format_count != 0 {
        ...
        vk.GetPhysicalDeviceSurfaceFormatsKHR(device, app.surface, &format_count, raw_data(result.formats))
    }
    ...
    if present_mode_count != 0 {
        ...
        vk.GetPhysicalDeviceSurfacePresentModesKHR(device, app.surface, &present_mode_count, raw_data(result.present_modes))
    }
    return
}
...
create_debug_messenger :: proc(instance: vk.Instance) -> (dm: vk.DebugUtilsMessengerEXT, ok : Dbg_Result = .SUCCESS) {
...
destroy_debug_messenger :: proc(instance: vk.Instance, dm: vk.DebugUtilsMessengerEXT, alloc_callback: ^vk.AllocationCallbacks) {
    if !enable_validation_layers {
        return
    }
...
debug_callback :: proc "system" (message_severity: vk.DebugUtilsMessageSeverityFlagsEXT, message_type: vk.DebugUtilsMessageTypeFlagsEXT, p_callback_data: ^vk.DebugUtilsMessengerCallbackDataEXT, p_user_data: rawptr) -> b32 {
